...
if (isset($_GET["mongo"])) {
	...
	if (class_exists('MongoDB\Driver\Manager')) {
		class Db extends SqlDb {
			...
			function attach($server, $username, $password): string {
				...
				// Use MongoDB\Client not Driver\Manager
				$this->_link = new \MongoDB\Client("mongodb://$server", ['username' => $username, 'password' => $password]);
			
				// Set database object
				if ($db != "") {
					$this->_db_name = $db;
					$this->_db = $this->_link->selectDatabase($db);
				}
			...
			function executeDbCommand($db, $command) {
				try {
					...
					// Set db name and object if not already
					if (!$this->_db || $this->_db_name !== $db) {
						$this->_db_name = $db;
						$this->_db = $this->_link->selectDatabase($db);
					}
			...
			function select_db($database) {
				...
				$this->_db = $this->_link->selectDatabase($database);
				return true;
			}
	...
	class Driver extends SqlDriver {
		...
		function select($table, array $select, array $where, array $group, array $order = array(), $limit = 1, $page = 0, $print = false) {
			$selectFields = ($select == array("*")) ? [] : array_fill_keys($select, 1);
			if (!isset($selectFields['_id'])) {
				$selectFields['_id'] = 0;
			}
			...
			foreach ($order as $val) {
				$val = preg_replace('~ DESC$~', '', $val, 1, $desc);
				$sort[$val] = ($desc ? -1 : 1);
			}
			...
			try {
				$cursor = $this->conn->_db->selectCollection($table)->find(
					$where,
					[
